...
router.get("/users/registrationbymonth", authorization, async (req, res) => {
    try {
        const usersData = await pool.query("SELECT user_location FROM users WHERE(MONTH FROM order_date)");
        res.json(usersData.rows)
    } catch (err) {
        console.error(err.message);
        res.status(500).send("Server Error")
